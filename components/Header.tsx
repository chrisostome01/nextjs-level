...
import { CREATENEWCARD, LOGIN } from '../pages/api/graphql';
import { Header } from '../utils/types/types';
import Topic from './Topic';


const  Header = ({topicName, topicId}:Header) => {
    const {systemTheme , theme, setTheme} = useTheme();
    const [ showLoginMode , setShowLoginMode  ] = useState(false);
    const [ showNewMode , setShowNewCardMode  ] = useState(false);
    const [signin] = useMutation(LOGIN);
    const [newCard] = useMutation(CREATENEWCARD);
    const changeTheme= () => {
    ...
    const createNewCard = async (e:any) => {
      ...
      if(topicId == 0){
        return toast('Please pick a topic to create a new card for', {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      try {
        let topicid =  topicId;
        const card = await newCard({ variables: { topicId:topicid ,title ,description } });         
        setShowNewCardMode(false); 
      } catch (error) {
        console.log(error);
        
        return toast('Please try login, again please', {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
    ...
    return (
        <header className="h-15 shadow-2xl dark:bg-black-800">
          ...
          <div className={`overlay bg-slate-900 opacity-70  h-full w-full absolute right-0 left-0 top-0  ${ showNewMode == true ? " flex" : " hidden"} items-center justify-center z-50`}>
            <div className="card w-4/12 min-h-2/6 min-2xl:h-60 bg-white opacity-100 dark:bg-gray-800 rounded-md shadow-xl text-center p-3 mx-4 flex flex-col items-center justify-center z-60">
              ...
              <div className="text-gray-500 font-medium min-w-full">
                <form className="w-full" onSubmit={(e) =>{
                  ...
                  <div className="input m-3">
                    <input type="text" name="title" className='text-center shadow-lg w-full p-2 rounded-md bg-blue-100' placeholder='Title'/>
                  </div>
                  <div className="input m-3">
                    <input type="text" name="description" className='text-center shadow-lg w-full p-2 rounded-md bg-blue-100' placeholder='Description'/>
                  </div>
